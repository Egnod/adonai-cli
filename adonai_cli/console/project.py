...
class ProjectMenu:
    ...
    @client_injection
    def create(self, name: str, domain_id: int, description: str = Defaults.NO_INPUT, client: AdonaiClient = None):
        mutation = client.mutation()

        args = get_arguments(name=name, description=description, domain_id=domain_id)
        client.fields(mutation.create_project(**args).project(), **get_fields_dict(self.DEFAULT_FIELDS))
    ...
    def update(self, id: int, name: str = Defaults.NO_INPUT, description: str = Defaults.NO_INPUT, client: AdonaiClient = None):
        ...
        client.fields(mutation.update_project(id=id, **args).project(), **get_fields_dict(self.DEFAULT_FIELDS))
    ...
    def toggle(self, id: int, is_active: bool, client: AdonaiClient = None):
        ...
        client.fields(mutation.toggle_project(id=id, **args).project(), **get_fields_dict(self.DEFAULT_FIELDS))
    ...
    def domain(self, id: int, client: AdonaiClient = None):
        ...
        client.fields(query.get_project(id=id).domain(), **get_fields_dict(DomainMenu.DEFAULT_FIELDS))
    ...
    def permissions(self, id: int, client: AdonaiClient = None):
        ...
        client.fields(query.get_project(id=id).permissions(), **get_fields_dict(PermissionMenu.DEFAULT_FIELDS))
